...
def upgrade() -> None:
    ...
    # Add nullable sectionId column to accessories table if it doesn't exist
    inspector = inspect(bind)  # Refresh inspector to check for new tables
    if 'accessories' in tables:
        columns = [col['name'] for col in inspector.get_columns('accessories')]
    ...
    if 'sectionConnections' not in tables:
        op.create_table(
            'sectionConnections',
            sa.Column('id', sa.Integer(), nullable=False),
